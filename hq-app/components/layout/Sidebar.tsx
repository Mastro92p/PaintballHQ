...
export function Sidebar({ isAdmin, userEmail }: Props) {
  ...
  const linkClass = (href: string) =>
    `flex items-center rounded-md text-sm transition-colors
     ${collapsed ? "justify-center px-0 py-2.5 gap-0" : "gap-2.5 px-2 py-2"}
     ${isActive(href)
  ...
  return (
    <aside
      className="shrink-0 h-dvh sticky top-0 flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transition-[width] duration-200 ease-in-out overflow-hidden"
    ...
    >
      ...
      <div className={`pt-4 pb-2 ${collapsed ? "px-2" : "px-3"}`}>
        ...
        {collapsed && <div className="h-[18px] mb-1" />}
      ...
      {/* Footer */}
      <div className={`py-3 border-t border-gray-200 dark:border-gray-800 shrink-0 ${collapsed ? "px-2" : "px-3"}`}>
        {isAdmin && userEmail ? (
          <div className="flex flex-col gap-1.5">
            {collapsed ? (
              <div className="flex justify-center mb-1">
                <div className="w-6 h-6 rounded-full bg-teal-600 flex items-center justify-center text-white text-[10px] font-bold" title={userEmail}>
          ...
          <Link
            href="/login"
            title={collapsed ? "Organizer Login" : undefined}
