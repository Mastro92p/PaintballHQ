...
export function MatchCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-gray-200 bg-gray-100 dark:border-white/10 dark:bg-[#0f172a] px-4 py-3">
      {/* Left image panel */}
      {match.teamA?.logoUrl && (
        <div
          className="pointer-events-none absolute inset-y-0 left-0 w-[43%] overflow-hidden bg-gray-200 dark:bg-slate-700"
          style={{
      ...
      {match.teamB?.logoUrl && (
        <div
          className="pointer-events-none absolute inset-y-0 right-0 w-[43%] overflow-hidden bg-gray-200 dark:bg-slate-700"
          style={{
      ...
      <div className="pointer-events-none absolute inset-y-2 left-1/2 w-[22%] -translate-x-1/2 rounded-md bg-black/4 dark:bg-black/6 blur-sm" />
      ...
      <div className="relative z-10">
        <div className="relative mb-2 h-4">
          <span className="absolute inset-x-0 top-0 text-center text-xs text-gray-500 dark:text-slate-400">
        ...
        <div className="flex items-center gap-3">
          <div className="flex min-w-0 flex-1 items-center">
            <span
              className={[
                "inline-flex max-w-full items-center rounded-full border border-gray-200 dark:border-white/10 bg-white/70 dark:bg-slate-950/55 px-2.5 py-1 text-sm backdrop-blur-sm",
                aWins ? "font-bold text-emerald-600 dark:text-emerald-300" : "font-medium text-gray-900 dark:text-white",
              ].join(" ")}
            >
          ...
          {completed ? (
            <div className="relative z-10 flex shrink-0 items-center gap-1.5">
              {isClassic && match.bodyCountA != null && (
                <span className="flex h-6 min-w-6 items-center justify-center rounded-full border border-gray-200 dark:border-white/10 bg-gray-200 dark:bg-slate-800/100 px-1.5 text-[10px] font-semibold leading-none text-gray-700 dark:text-slate-100 tabular-nums">
                  {match.bodyCountA}
                </span>
              )}
            ...
            <span className="shrink-0 px-2 text-xs font-medium text-gray-500 dark:text-slate-400">
              vs
            </span>
          )}
          ...
          <div className="flex min-w-0 flex-1 items-center justify-end">
            <span
              className={[
                "inline-flex max-w-full items-center rounded-full border border-gray-200 dark:border-white/10 bg-white/70 dark:bg-slate-950/55 px-2.5 py-1 text-sm backdrop-blur-sm",
                bWins ? "font-bold text-emerald-600 dark:text-emerald-300" : "font-medium text-gray-900 dark:text-white",
              ].join(" ")}
            >
