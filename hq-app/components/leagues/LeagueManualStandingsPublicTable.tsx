...
function ScoreRankCell({
  ...
  if (score == null && rank == null) {
    return (
      <div className="px-2 py-2 text-center text-[11px] text-gray-400 dark:text-gray-500">
        —
      </div>
    );
  }
  ...
  return (
    <div className="px-2 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-800/60 min-h-[52px] flex flex-col justify-center">
      <div className="text-[13px] leading-none font-semibold tabular-nums text-gray-900 dark:text-gray-100">
        {score ?? "—"}
      </div>
      <div className="mt-1 text-[10px] leading-none text-gray-500 dark:text-gray-400">
        Rank {rank ?? "—"}
      </div>
    </div>
  );
      ...
      <div className="overflow-x-hidden">
        <table className="w-full table-fixed text-xs">
          <thead className="bg-slate-800 dark:bg-slate-900">
            <tr>
              <th className="w-[220px] px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wide text-slate-300">
                Team
              </th>
              ...
              {days.map((day) => (
                <th
                  key={day.id}
                  className="px-2 py-2 text-left text-[10px] font-semibold text-slate-300"
                >
                  <div className="leading-tight min-w-0">
                    <div className="text-[11px] font-semibold text-slate-100 line-clamp-3 break-words">
              ...
              <th className="w-[88px] px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-300">
                Total
              </th>
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {rows.map((row) => {
              ...
              const rowTone = isGold
                ...
                : isSilver
                ? "bg-slate-300/18 dark:bg-slate-300/17" //bg-zinc-300/18
                : isBronze
