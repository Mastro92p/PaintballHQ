...
export async function POST(
  ...
  try {
    ...
    const pathname = `leagues/${id}/logo-${Date.now()}.${ext}`;
    ...
    const previousLogoUrl = league.logoUrl;
    ...
    return apiError("Failed to upload league logo", 500);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return apiError("Failed to remove league logo", 500);
  }
}